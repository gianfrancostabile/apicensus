...
public class PostPersonController extends MainController {
    ...
    public void configure() throws Exception {
        ...
        rest()
            ...
            .post("/").outType(ResponseListDTO.class)
            .description("Returns the person information by ssn")
            // Defines the header name, type, and allowed values to Swagger
            .param()
                .name("country").type(RestParamType.header)
                .dataType("string").description("The country of people to look for")
                .allowableValues(
                    Stream.of(Country.values()).map(country -> country.name())
                        .collect(Collectors.toList())).required(true)
            .endParam()
            // Defines the body type to Swagger
            .type(Object[].class)
